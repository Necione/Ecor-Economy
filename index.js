...
client.on("messageCreate", async (message) => {
  ...
  if (
    ...
    if (xxx == 2) {
      ...
      embed = new MessageEmbed()
        .setColor("ORANGE")
        .setTitle(`⭐ Random Event | Type Racer`)
        .setImage(word.image)
        .setDescription(`First to type everything wins 50 coins!`)
        .setFooter({ text: `Caramel Iced Latte` });
      message.channel.send({ embeds: [embed] });
      ...
      collector.on("end", (collected) => {
        if (fail == true) {
          ...
          for (i in participated) {
            ...
            if (!isLocked(participated[i])) dz.balance += 10;
          ...
          embed = new MessageEmbed()
            .setColor("ORANGE")
            .setTitle(`Game over!`)
            .setDescription(`Each user has been awarded 10 coins per number counted!`);
          message.channel.send({ embeds: [embed] });
        }
      });
    }
